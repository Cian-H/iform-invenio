...
def main():
    ...
    if os.path.exists(global_ga):
        try:
            ...
            if "invenio_communities" in ga_data:
                ...
                print(
                    f"Extracted invenio_communities translations to {communities_dst}"
                )
    ...
    files = glob.glob(
        "/opt/invenio/src/.venv/lib/python3.12/site-packages/**/_generatedTranslations.js",
        recursive=True,
    ) + glob.glob(
        "/opt/invenio/var/instance/assets/**/_generatedTranslations.js", recursive=True
    )
    for f in files:
        with open(f, "r") as fp:
            content = fp.read()
        if "TRANSLATE_GA" not in content:
            ...
            if last_import != -1:
                lines.insert(
                    last_import + 1,
                    'import TRANSLATE_GA from "./ga/translations.json";',
                )
            content = "\n".join(lines)
